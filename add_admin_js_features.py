...
async function sendAdminBuddy() {
  ...
  try {
    ...
    errorMsg.textContent = "Sorry, I couldn't process that right now. (Simulated AI Error or API unlinked)";
...
function initLiveFeed() {
  ...
  setInterval(() => {
    ...
    el.style.cssText = "padding: 8px 12px; background: rgba(255,255,255,0.6); border-radius: 8px; font-size: 0.85rem; color: #166534; animation: slideDown 0.3s ease-out;";
...
// Add animation keyframes for the ticker
...
# Inject renderAdminCharts into loadClassAnalytics
js = js.replace('document.getElementById("analyticsAvgScore").textContent = avgAcc;', 'document.getElementById("analyticsAvgScore").textContent = avgAcc;\n  if(typeof renderAdminCharts === "function") renderAdminCharts();')

# Inject initLiveFeed into initAdmin
js = js.replace('loadProgressData();', 'loadProgressData();\n  if(typeof initLiveFeed === "function") initLiveFeed();')

# Append all features to the end of the file
if 'toggleAdminBuddy' not in js:
    js += '\n' + features_js

with open('admin.js', 'w') as f:
    f.write(js)

print("Admin JS features added!")
